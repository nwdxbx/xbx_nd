...
io.imsave('{}/color_{}.jpg'.format(save_folder, name), np.squeeze(color_image))
...
z = z/np.max(z)
attribute = z
depth_image = mesh_numpy.render.render_colors(image_vertices, triangles, attribute, h, w, c=1)
io.imsave('{}/depth_{}.jpg'.format(save_folder, name), np.squeeze(depth_image))
...
attribute = pncc
pncc_image = mesh_numpy.render.render_colors(image_vertices, triangles, attribute, h, w, c=3)
io.imsave('{}/pncc_{}.jpg'.format(save_folder, name), np.squeeze(pncc_image))
...
uv_coords_image = mesh_numpy.render.render_colors(image_vertices, triangles, attribute, h, w, c=2) # two channels: u, v
# add one channel for show
uv_coords_image = np.concatenate((np.zeros((h, w, 1)), uv_coords_image), 2)
io.imsave('{}/uv_coords_{}.jpg'.format(save_folder, name), np.squeeze(uv_coords_image))
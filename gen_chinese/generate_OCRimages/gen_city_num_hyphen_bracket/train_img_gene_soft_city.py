...
def gene_img(text):
    ...
    line = textwrap.fill(text[0:lenroad],width = lenroad)
    ...
    font = ImageFont.truetype(font_path, font_size)
    '''
    line = textwrap.fill(text[lenroad:],width = len(text)-lenroad)
    draw.text((start_x+font_size*lenroad,start_y+4),line,(random.randint(20,50),random.randint(20,50),random.randint(20,50)),font=font)
    '''
    stride_x = font_size - 4 
    for idx,char in enumerate(text[lenroad:]):
        x_min = start_x*(lenroad) + (idx+1) * stride_x + 30
        draw.text(( x_min , start_y ), char,(random.randint(20,50),random.randint(20,50),random.randint(20,50)) ,font=font)
    del draw

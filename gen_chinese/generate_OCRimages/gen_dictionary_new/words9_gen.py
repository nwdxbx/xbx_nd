...
def gene_img(text):
    ...
    img_r = gaussianNoisy(img[:, :, 0].flatten())+random.randint(-30,80)
    img_g = gaussianNoisy(img[:, :, 1].flatten())+random.randint(-30,80)
    img_b = gaussianNoisy(img[:, :, 2].flatten())+random.randint(-30,80)
    ...
    start_x, start_y = random.randint(5,25),random.randint(7,10)
    if sdsds > 1:
        start_y = random.randint(7,9)
    elif sdsds < -1:
        start_y = random.randint(9,11)
    for idx,char in enumerate(char_list):
        x_min = start_x + i * stride_x + random.randint(-1,1)
        y_min = start_y + j * stride_y +random.randint(-1,1) 
        draw.text(( x_min , y_min ), char,(random.randint(0,50),random.randint(0,50),random.randint(0,50)) ,font=font)
    ...
    k = random.choice([[3,1],[5,1]])
    #result_im = cv2.GaussianBlur(result_im ,( k[0], k[0] ),k[1])
    blur_im = result_im

    #blur locally
    for i in range(random.randint(0,20)):
        randx = random.randint(0,220)
        randy = random.randint(0,30)
        randy_end = randy+random.randint(1,8)
        randx_end = randx+random.randint(1,18)
        blur_im[randy:randy_end,randx:randx_end] = cv2.GaussianBlur(result_im[randy:randy_end,randx:randx_end],(k[0],k[0]),2)

...
class BackgroundGenerator(object):
    ...
    @classmethod
    def gaussian_noise_RGB(cls,height,width):
        image = np.ones((height,width,3))
        tmp = np.ones((height,width))*60
        cv2.randn(tmp,50,10)
        image[:,:,0]=tmp
        tmp = np.ones((height,width))*255
        cv2.randn(tmp,235,10)
        image[:,:,1]=tmp
        tmp = np.ones((height,width))*80
        cv2.randn(tmp,100,20)
        image[:,:,2] = tmp

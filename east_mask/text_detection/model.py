    ...
    with tf.variable_scope('feature_fusion', values=[end_points.values]):
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            f = [end_points['pool5'], end_points['pool4'],
                 end_points['pool3'], end_points['pool2']]   #feature layers from resnet
            g = [None, None, None, None]
            h = [None, None, None, None]
            num_outputs = [None, 128, 64, 32]
            for i in range(4):
                if i == 0:
                    h[i] = f[i]   #h[0] = f[0]   dim:(?,?,?,2048)
                else:
...
def distance_balanced_sigmoid_crossentrop(y_true_cls, y_pred_cls,y_true_geo,training_mask):
    eps = 1e-5
    ...
    #loss = -1*tf.reduce_mean(beta * labels * tf.log(logits + eps)*(1-min_distance) + (1 - beta) * (1 - labels) * tf.log(1 - logits + eps))
...
def loss(y_true_cls, y_pred_cls,
    ...
    #classification_loss = distance_balanced_sigmoid_crossentrop(y_true_cls, y_pred_cls,y_true_geo,training_mask)
    # scale classification loss to match the iou loss part
    #classification_loss *= 0.01  #for dice loss

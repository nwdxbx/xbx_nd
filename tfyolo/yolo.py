...
from yolo3.utils import padbox_image
class YOLO(object):
    def __init__(self):
    ...
    def generate(self):
        ...
        hsv_tuples = [(1.0*x / len(self.class_names), 1., 1.)
    ...
    def detect_image(self, image):
        ...
        print ('......',list(out_classes),'aaaaaaa',reversed(list(enumerate(out_classes))))
    ...
    def get_boxes_scores(self, image):


        if self.model_image_size != (None, None):
            assert self.model_image_size[0]%16 == 0, 'Multiples of 32 required'
            assert self.model_image_size[1]%16 == 0, 'Multiples of 32 required'
            boxed_image = padbox_image(image,self.model_image_size)
            # boxed_image = letterbox_image(image, tuple(reversed(self.model_image_size)))
        else:
            new_image_size = (image.width - (image.width % 16),
                              image.height - (image.height % 16))
...
def detect_show_img(yolo):
    ...
    for filename in files:
        ...
        try:
            ...
            boxes,classes,scores = yolo.get_boxes_scores(image)
